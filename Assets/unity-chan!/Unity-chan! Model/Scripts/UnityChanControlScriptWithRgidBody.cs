...
// 必要なコンポーネントの列記
	...
	public class UnityChanControlScriptWithRgidBody : MonoBehaviourPunCallbacks, IPunObservable
	{
		...
		void FixedUpdate ()
		{
			...
			float nowPosy = this.gameObject.transform.position.z;
			float nextPosx = GameManager.Instance.cardHolders[nowy].CardPoints[nowx].transform.position.x;
			float nextPosy = GameManager.Instance.cardHolders[nowy].CardPoints[nowx].transform.position.z;
			...
			else if (nextPosx < nowPosx)
			{
				this.gameObject.transform.rotation = Quaternion.Euler(0, -90, 0);
				rb.velocity = new Vector3(-15f, 0, 0);
			}
			else if (nextPosy > nowPosy)
			{
			...
			Debug.LogError(nextPosx.ToString() + " " + nextPosy.ToString() + " " + this.gameObject.transform.position.x.ToString() + " " + this.gameObject.transform.position.z.ToString());
			if (Mathf.Abs(this.gameObject.transform.position.x - nextPosx) > 0.5f || Mathf.Abs(this.gameObject.transform.position.z - nextPosy) > 0.5f) return;
			rb.velocity = Vector3.zero;
			this.gameObject.transform.position = new Vector3(nextPosx, this.gameObject.transform.position.y, nextPosy);
			nowRoot++;
